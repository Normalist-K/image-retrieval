...
from net.gem_pool import GeM
from .simclr import get_resnet, name_to_params
...
class DolgNet(nn.Module):
    def __init__(self,
        ...
        if model_name == 'resnet50_simclr':
            ...
            self.model, _ = get_resnet(*name_to_params(checkpoint))
            self.model.load_state_dict(torch.load(checkpoint)['resnet'])
        else:
